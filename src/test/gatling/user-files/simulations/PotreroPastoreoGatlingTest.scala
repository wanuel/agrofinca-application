...
/**
 * Performance test for the PotreroPastoreo entity.
 */
class PotreroPastoreoGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the PotreroPastoreo entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all potreroPastoreos")
            .get("/api/potrero-pastoreos")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new potreroPastoreo")
            .post("/api/potrero-pastoreos")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "fechaIngreso":"2020-01-01T00:00:00.000Z"
                , "fechaSalida":"2020-01-01T00:00:00.000Z"
                , "fechaLimpia":"2020-01-01T00:00:00.000Z"
                , "diasDescanso":"0"
                , "diasCarga":"0"
                , "limpia":"SI"
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_potreroPastoreo_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created potreroPastoreo")
                .get("${new_potreroPastoreo_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created potreroPastoreo")
            .delete("${new_potreroPastoreo_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
