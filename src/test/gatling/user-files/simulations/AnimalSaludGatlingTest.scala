...
/**
 * Performance test for the AnimalSalud entity.
 */
class AnimalSaludGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the AnimalSalud entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all animalSaluds")
            .get("/api/animal-saluds")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new animalSalud")
            .post("/api/animal-saluds")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "fecha":"2020-01-01T00:00:00.000Z"
                , "nombre":"SAMPLE_TEXT"
                , "laboratorio":"SAMPLE_TEXT"
                , "dosis":"0"
                , "inyectado":"SI"
                , "intramuscular":"SI"
                , "subcutaneo":"SI"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_animalSalud_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created animalSalud")
                .get("${new_animalSalud_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created animalSalud")
            .delete("${new_animalSalud_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
