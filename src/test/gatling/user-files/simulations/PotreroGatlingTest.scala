...
/**
 * Performance test for the Potrero entity.
 */
class PotreroGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Potrero entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all potreros")
            .get("/api/potreros")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new potrero")
            .post("/api/potreros")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_potrero_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created potrero")
                .get("${new_potrero_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created potrero")
            .delete("${new_potrero_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
