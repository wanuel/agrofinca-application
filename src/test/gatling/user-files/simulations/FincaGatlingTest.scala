...
/**
 * Performance test for the Finca entity.
 */
class FincaGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Finca entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all fincas")
            .get("/api/fincas")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new finca")
            .post("/api/fincas")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                ...
                , "codigoCatastral":"SAMPLE_TEXT"
                , "municipio":"SAMPLE_TEXT"
                , "vereda":"SAMPLE_TEXT"
                , "obserrvaciones":"SAMPLE_TEXT"
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_finca_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created finca")
                .get("${new_finca_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created finca")
            .delete("${new_finca_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
