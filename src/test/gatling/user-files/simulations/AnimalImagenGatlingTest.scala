...
/**
 * Performance test for the AnimalImagen entity.
 */
class AnimalImagenGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the AnimalImagen entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all animalImagens")
            .get("/api/animal-imagens")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new animalImagen")
            .post("/api/animal-imagens")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_animalImagen_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created animalImagen")
                .get("${new_animalImagen_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created animalImagen")
            .delete("${new_animalImagen_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
