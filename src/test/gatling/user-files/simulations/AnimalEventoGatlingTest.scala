...
/**
 * Performance test for the AnimalEvento entity.
 */
class AnimalEventoGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the AnimalEvento entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all animalEventos")
            .get("/api/animal-eventos")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new animalEvento")
            .post("/api/animal-eventos")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_animalEvento_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created animalEvento")
                .get("${new_animalEvento_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created animalEvento")
            .delete("${new_animalEvento_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
