...
/**
 * Performance test for the AnimalCostos entity.
 */
class AnimalCostosGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the AnimalCostos entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all animalCostos")
            .get("/api/animal-costos")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new animalCostos")
            .post("/api/animal-costos")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_animalCostos_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created animalCostos")
                .get("${new_animalCostos_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created animalCostos")
            .delete("${new_animalCostos_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
