...
/**
 * Performance test for the AnimalPeso entity.
 */
class AnimalPesoGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the AnimalPeso entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all animalPesos")
            .get("/api/animal-pesos")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new animalPeso")
            .post("/api/animal-pesos")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_animalPeso_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created animalPeso")
                .get("${new_animalPeso_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created animalPeso")
            .delete("${new_animalPeso_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
