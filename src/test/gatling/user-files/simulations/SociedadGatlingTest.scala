...
/**
 * Performance test for the Sociedad entity.
 */
class SociedadGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Sociedad entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all sociedads")
            .get("/api/sociedads")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new sociedad")
            .post("/api/sociedads")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_sociedad_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created sociedad")
                .get("${new_sociedad_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created sociedad")
            .delete("${new_sociedad_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
