...
/**
 * Performance test for the Parametros entity.
 */
class ParametrosGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Parametros entity")
        ...
        .repeat(2) {
            ...
            .exec(http("Create new parametros")
            .post("/api/parametros")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_parametros_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created parametros")
                .get("${new_parametros_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created parametros")
            .delete("${new_parametros_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
