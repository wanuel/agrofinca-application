...
/**
 * Performance test for the AnimalLote entity.
 */
class AnimalLoteGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the AnimalLote entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all animalLotes")
            .get("/api/animal-lotes")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new animalLote")
            .post("/api/animal-lotes")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_animalLote_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created animalLote")
                .get("${new_animalLote_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created animalLote")
            .delete("${new_animalLote_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
