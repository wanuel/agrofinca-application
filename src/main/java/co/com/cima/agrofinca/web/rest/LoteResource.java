...
public class LoteResource {
    ...
    private final LoteQueryService loteQueryService;

    public LoteResource(LoteService loteService, LoteQueryService loteQueryService) {
        this.loteService = loteService;
        this.loteQueryService = loteQueryService;
    }
     ...
     */
    @GetMapping("/lotes")
    public ResponseEntity<List<Lote>> getAllLotes(LoteCriteria criteria, Pageable pageable) {
        log.debug("REST request to get Lotes by criteria: {}", criteria);
        Page<Lote> page = loteQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /lotes/count} : count all the lotes.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/lotes/count")
    public ResponseEntity<Long> countLotes(LoteCriteria criteria) {
        log.debug("REST request to count Lotes by criteria: {}", criteria);
        return ResponseEntity.ok().body(loteQueryService.countByCriteria(criteria));
    }
