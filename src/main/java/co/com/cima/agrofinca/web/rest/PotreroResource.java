...
public class PotreroResource {
    ...
    private final PotreroQueryService potreroQueryService;

    public PotreroResource(PotreroService potreroService, PotreroQueryService potreroQueryService) {
        this.potreroService = potreroService;
        this.potreroQueryService = potreroQueryService;
    }
     ...
     */
    @GetMapping("/potreros")
    public ResponseEntity<List<Potrero>> getAllPotreros(PotreroCriteria criteria, Pageable pageable) {
        log.debug("REST request to get Potreros by criteria: {}", criteria);
        Page<Potrero> page = potreroQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /potreros/count} : count all the potreros.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/potreros/count")
    public ResponseEntity<Long> countPotreros(PotreroCriteria criteria) {
        log.debug("REST request to count Potreros by criteria: {}", criteria);
        return ResponseEntity.ok().body(potreroQueryService.countByCriteria(criteria));
    }
