...
public class AnimalImagenResource {
    ...
    private final AnimalImagenQueryService animalImagenQueryService;

    public AnimalImagenResource(AnimalImagenService animalImagenService, AnimalImagenQueryService animalImagenQueryService) {
        this.animalImagenService = animalImagenService;
        this.animalImagenQueryService = animalImagenQueryService;
    }
    ...
    public ResponseEntity<List<AnimalImagen>> getAllAnimalImagens(AnimalImagenCriteria criteria, Pageable pageable) {
        log.debug("REST request to get AnimalImagens by criteria: {}", criteria);
        Page<AnimalImagen> page = animalImagenQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /animal-imagens/count} : count all the animalImagens.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/animal-imagens/count")
    public ResponseEntity<Long> countAnimalImagens(AnimalImagenCriteria criteria) {
        log.debug("REST request to count AnimalImagens by criteria: {}", criteria);
        return ResponseEntity.ok().body(animalImagenQueryService.countByCriteria(criteria));
    }
