...
public class AnimalSaludResource {
    ...
    private final AnimalSaludQueryService animalSaludQueryService;

    public AnimalSaludResource(AnimalSaludService animalSaludService, AnimalSaludQueryService animalSaludQueryService) {
        this.animalSaludService = animalSaludService;
        this.animalSaludQueryService = animalSaludQueryService;
    }
    ...
    public ResponseEntity<List<AnimalSalud>> getAllAnimalSaluds(AnimalSaludCriteria criteria, Pageable pageable) {
        log.debug("REST request to get AnimalSaluds by criteria: {}", criteria);
        Page<AnimalSalud> page = animalSaludQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /animal-saluds/count} : count all the animalSaluds.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/animal-saluds/count")
    public ResponseEntity<Long> countAnimalSaluds(AnimalSaludCriteria criteria) {
        log.debug("REST request to count AnimalSaluds by criteria: {}", criteria);
        return ResponseEntity.ok().body(animalSaludQueryService.countByCriteria(criteria));
    }
